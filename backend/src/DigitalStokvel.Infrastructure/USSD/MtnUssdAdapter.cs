...
/// <summary>
/// USSD gateway adapter for MTN network (stub implementation for MVP)
/// </summary>
public class MtnUssdAdapter : IUssdGateway
{
    private readonly ILogger<MtnUssdAdapter> _logger;
    public string ProviderName => "MTN";

    public MtnUssdAdapter(ILogger<MtnUssdAdapter> logger)
    {
    ...
    {
        _logger.LogInformation("[MTN STUB] Sending USSD menu | Session: {SessionId}", sessionId);
    ...
    {
        _logger.LogInformation("[MTN STUB] Processing input | Session: {SessionId} | Input: {UserInput}", sessionId, userInput);
        await Task.Delay(50, cancellationToken);
    ...
    {
        _logger.LogInformation("[MTN STUB] Managing session | Session: {SessionId} | Operation: {Operation}", sessionId, operation);
